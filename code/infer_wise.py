"""Generate images for WISE benchmark evaluation using Mask-GRPO / Show-O.
...
def parse_args():
    ...
    parser.add_argument("--data_path", type=str, required=True,
                        help="Path to wise_eval_data.json")
    parser.add_argument("--save_dir", type=str, required=True,
                        help="Directory to save generated images")
    parser.add_argument("--showo_model_path", type=str, default="showo.pth",
                        help="Path to the base showo.pth model")
    parser.add_argument("--checkpoint", type=str, default="",
                        help="Path to trained checkpoint (state_dict). Empty = use base model only")
...
def main():
    ...
    # --- Build all tasks: (prompt_id, gen_idx, prompt, out_path) ---
    all_tasks = []
    for idx, item in enumerate(wise_data):
        prompt = item["Prompt"]
        prompt_id = item.get("prompt_id", idx)
        for gen_idx in range(args.num_generation):
            filename = f"ID{prompt_id}_{gen_idx}.jpg"
            out_path = os.path.join(args.save_dir, filename)
            all_tasks.append((prompt, out_path))

    # --- Shard across GPUs ---
    my_tasks = all_tasks[rank::world_size]

    skipped = sum(1 for _, p in my_tasks if os.path.exists(p))
    ...
    if rank == 0:
        print(f"[INFO] All GPUs done. {len(all_tasks)} images saved to {args.save_dir}")
